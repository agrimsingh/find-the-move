...
- Spoken arc is exactly 10 slides: `title`, `let-it-wander`, `opened-a-new-chat`, `thesis`, `steps-in-a-file`, `the-test-stays`, `same-path-in-the-repo`, `copied-from-the-chat`, `deleted-the-recipe`, `dont-merge`.
- Card titles and bodies in `lib/slides.tsx` are locked copy. `steps-in-a-file` and `the-test-stays` are title plus the file. `copied-from-the-chat` is a code-split of the chat log versus the skill file. No PartyKit. No AI images.
